...
private["_maxheight", "_sessionID", "_parameters", "_netID", "_nickname", "_playerObject", "_vehicleObject", "_alphabet", "_forbiddenCharacter", "_flagObject", "_storedVehicles", "_level", "_maxNumberOfVehicles", "_vehiclePosition", "_items", "_magazines", "_weapons", "_containers", "_popTabs", "_groundHolder", "_popTabsObject", "_rearmVehs"];
...
try
{
	...
	// Prevent vehicle storage above a certain height.
	
	private _vehicleObjectHeightASL = (getPosASL _vehicleObject) select 2;
	private _vehicleObjectHeightATL = (getPosATL _vehicleObject) select 2;
	private _vehicleObjectHeight = 0;
	
	if (_vehicleObjectHeightASL <= _vehicleObjectHeightATL) then
	{
		_vehicleObjectHeight = _vehicleObjectHeightASL;
	} 
	else
	{
		_vehicleObjectHeight = _vehicleObjectHeightATL;
	};
	
	//private _vehicleObjectHeight = (getPosATL _vehicleObject) select 2;
	if ((_vehicleObjectHeight >= _maxheight) && !(_vehicleObject isKindof "Air")) then 
	{
		throw format ["You cannot store this kind of vehicle above the height of %1M!", _maxheight];
	};
	// End.
